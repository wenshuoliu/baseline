...
class MAPModel(nn.Module):
    ...
    def __init__(self, encoder, dropout=0.0, weight_std=0.02):
        ...
        self.top_layer = pytorch_linear(self.d_model, self.d_model, bias=False)
    ...
    def create_loss(self):
        return AllLoss(self, warmup_steps=200)
...
def run():
    ...
    parser.add_argument("--warmup_steps", type=int, default=20, help="Num warmup steps")
    ...
    preproc_data = baseline.embeddings.load_embeddings('encoder',
                                                       ...
                                                       embed_file=embeddings_config['file'],
    ...
    reader = MultiFileDatasetReader(args.nctx, args.subword_model_file, args.subword_vocab_file, args.pattern,
                                    reader_type='ntp', vectorizer_begin_tok='[CLS]', vectorizer_end_tok='<EOU>')
    train_set = reader.load(args.train_file, vocab, distribute=False, shuffle=True)
    test_set = reader.load(args.test_file, vocab, distribute=False, shuffle=False)
