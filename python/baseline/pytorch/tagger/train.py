...
class TaggerTrainerPyTorch(EpochReportingTrainer):
    ...
    def _test(self, ts, **kwargs):
        ...
        for batch_dict in ts:
            ...
            batch = self.model.make_input(batch_dict)
            inputs = (batch['x'], batch['xch'], batch['lengths'])
            pred = self.model(inputs)
            correct, count, overlaps, golds, guesses = self.process_output(pred, batch['y'].data, batch['lengths'], batch['ids'], handle, txts)
